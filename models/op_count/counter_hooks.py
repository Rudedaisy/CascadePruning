...
#   Code adapted from https://github.com/Lyken17/pytorch-OpCounter/
...
def count_convNd(m, x, y):
    ...
    if kernel_ops == 1:
        total_ops = x.shape[2] * x.shape[3] * torch.sum(m.weight!=0)
#        print(x.shape[1] * x.shape[2], torch.sum(m.weight!=0) )
    else:
...
def count_decomp_convNd(m, x, y):
    ...
    combing_ops = torch.sum(m.coefs!=0) * mid_size
