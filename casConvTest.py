...
model.load_state_dict(torch.load("./ckpt/VGG1603082154/retrain_weight_41_0.92.pt"))
model.cuda()
...
for quant in range(8,1,-1):
    ...
    for ars in range(2,34,2):
        ...
        for i in range(len(model.features)):
            ...
            if isinstance(model.features[i], PrunedConv):
                model.features[i] = casConv2d(model.features[i].conv, array_size=ars, quant=quant)
            if isinstance(model.features[i], casConv2d):
